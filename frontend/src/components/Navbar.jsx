...
function NavBar() {
    ...
    return (
        <div className="w-full h-[70px] flex justify-between items-center  z-20 font-text sticky top-0 bg-lightText p-5  ">
        <div className="flex items-center space-x-3 mx-1 md:mx-5">
            <div>
            <motion.button
                onClick={toggleMenu}
                whileTap={{ scale: 0.8 }}
                className="flex items-center border-2 md:px-3 md:py-2 px-2 py-1 rounded-full bg-btn1 cursor-pointer text-sm"
            >
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.ul
                    ...
                    exit="exit"
                    initial="initial"
                    className="absolute top-20 left-0 bg-btn2 justify-center text-justify bg-opacity-80"
                >
                    {menuItems.map((item) => (
                    <motion.li
                        to={item.link}
        ...
        <div className="">
            ...
            <motion.h1
                ...
                className="cursor-pointer  uppercase md:text-4xl text-3xl font-semibold tracking-wider font-logo text-btn1"
            >
                DI-PE
            </motion.h1>
            </Link>
        ...
        {loggedIn ? (
            ...
            <Link to="/signup">
                <motion.button
                whileTap={{ scale: 0.8 }}
                className="flex items-center border-2  md:mx-2 mx-1 md:px-3 md:py-2 px-2 py-1 rounded-full bg-btn1 cursor-pointer text-sm"
                >
                Register
                </motion.button>
            </Link>
